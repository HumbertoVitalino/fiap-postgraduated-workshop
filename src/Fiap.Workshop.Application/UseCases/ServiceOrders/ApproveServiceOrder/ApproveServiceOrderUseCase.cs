...
namespace Fiap.Workshop.Application.UseCases.ServiceOrders.ApproveServiceOrder;

public sealed class ApproveServiceOrderUseCase(
    IServiceOrderRepository serviceOrderRepository,
    IInventoryItemRepository inventoryItemRepository,
    ILogger<ApproveServiceOrderUseCase> logger
) : IApproveServiceOrderUseCase
{
    ...
    private readonly ILogger<ApproveServiceOrderUseCase> _logger = logger;

    public async Task<Output> Handle(ApproveServiceOrderInput input, CancellationToken cancellationToken)
    {
        Output output = new();
        ...
        foreach (var part in serviceOrder.Parts)
        {
            ...
            inventoryItem.CommitReservation(part.Quantity);
            inventoryItems.Add(inventoryItem);
        }

        serviceOrder.Approve(input.ChangedBy);
