...
> Documento vivo de contexto técnico. Atualize sempre que a arquitetura, os use cases ou as decisões de design mudarem. Última atualização: 2026-08-13 (branch `feature/10-update-customer`).
...
> ⚠️ Reescrita em 2026-08-08 (junto com §3), atualizada em 2026-08-11 com a chegada de `LoginUser` e `CreateCustomer`, em 2026-08-12 com `GetCustomer` e `GetUsers` (branch `feature/7-get-users`), no mesmo dia com `UpdateUser` e `ChangePassword` (branch `feature/8-put-users`), em 2026-08-13 com `DeleteUser` (branch `feature/9-user-delete`), e novamente no mesmo dia com `UpdateCustomer` (branch `feature/10-update-customer`). `UpdateEmailUseCase` (o scaffold antigo, só `Email`) continua sem existir — foi substituído em espírito pelo `UpdateUser` (Name/Email/Role) descrito abaixo.
...
### Use cases existentes (nove hoje: `CreateUser`, `LoginUser`, `CreateCustomer`, `GetCustomer`, `GetUsers`, `UpdateUser`, `ChangePassword`, `DeleteUser`, `UpdateCustomer`)
...
#### `UseCases/DeleteUser/` (novo, branch atual `feature/9-user-delete`)
...
#### `UseCases/UpdateCustomer/` (novo, branch atual `feature/10-update-customer`)
...
- `UseCases/ICreateUserUseCase`, `ILoginUserUseCase`, `ICreateCustomerUseCase`, `IGetCustomerUseCase`, `IGetUsersUseCase`, `IUpdateUserUseCase`, `IChangePasswordUseCase`, `IDeleteUserUseCase`, `IUpdateCustomerUseCase` — um por use case, todos só com `Handle(TInput, CancellationToken) : Task<Output>` (em `IGetUsersUseCase`, `TInput` é só `Guid correlationId`, sem boundary próprio — ver acima).
...
`AddApplication()` registra `ICreateUserUseCase`, `ILoginUserUseCase`, `ICreateCustomerUseCase`, `IGetCustomerUseCase`, `IGetUsersUseCase`, `IUpdateUserUseCase`, `IChangePasswordUseCase`, `IDeleteUserUseCase` e `IUpdateCustomerUseCase`, todos como `Scoped`.
...
> ⚠️ Reescrita em 2026-08-11 — desde a última passada (2026-08-08) ganhou o endpoint de login (`Auth`) e o primeiro endpoint de um agregado de negócio (`Customers`, criação). Atualizada em 2026-08-12 (branch `feature/8-put-users`) com `PUT /users/{id}` e `PATCH /users/{id}/password`, em 2026-08-13 (branch `feature/9-user-delete`) com `DELETE /users/{id}`, e novamente em 2026-08-13 (branch `feature/10-update-customer`) com `PUT /customers/{id}`. `GetUserById` (o antigo scaffold) continua sem existir como rota própria — `GetUsers` (lista) e `UpdateUser` cobrem o essencial que ele faria.
...
`CorrelationId` (`Guid`, `NotEmpty`) é um **campo do corpo** em todo request que tem corpo (`CreateUserRequest`, `LoginRequest`, `CreateCustomerRequest`, `UpdateUserRequest`, `ChangePasswordRequest`, `UpdateCustomerRequest` — `POST`/`PUT`/`PATCH`), não um header — não existe leitura de `x-correlation-id` em lugar nenhum do código atual. Nas rotas sem corpo (`GetUsers`, `GetCustomer`, `DELETE /users/{id}`), o padrão é diferente: o próprio endpoint gera um `Guid.NewGuid()` inline na chamada ao use case, o cliente não tem como fornecer/propagar um `CorrelationId` nessas rotas.
...
> ⚠️ Lista revisada em 2026-08-13 contra o código real (branch `feature/10-update-customer`) — login, `GetCustomer`/`GetUsers`, `UpdateUser`, `ChangePassword`, `DeleteUser` (CRUD de `User` completo) e agora `UpdateCustomer` já existem, mas o problema de bootstrap de Admin continua sem solução.
