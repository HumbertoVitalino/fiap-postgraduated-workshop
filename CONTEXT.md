...
- `AppDbContext` (implementa `IUnitOfWork`): `DbSet` para todos os agregados (`Users`, `Customers`, `Vehicles`, `InventoryItems`, `Services`, `ServiceOrders`); `CommitAsync` faz `SaveChangesAsync`, e se houver dispatcher e eventos pendentes, dispara `IDomainEventDispatcher.DispatchAsync` **após** o commit; captura exceções (inclusive as do dispatch) e retorna `false` — ver §11 para o problema que isso causa quando houver handlers registrados.
- `UserModel` (`Repositories/Models/`): `Id`, `Email`, `Name`, `Password` (hash, `varbinary(max)`), `Role`, `CreatedAt` (`DateTime`, não nulo), `UpdatedAt` (`DateTime?`). Ambos vêm do domínio via `MapToModel`/`MapToDomain` — não há mais default de banco (`GETUTCDATE()` foi removido); o `User` é a única fonte de verdade para essas datas.
- Mapeamento tabela `Users`: `Email` único (`HasIndex().IsUnique()`), `MaxLength(256)`; `Name` `MaxLength(100)`; `Role` `HasConversion<string>().HasMaxLength(20)` (corrigido em 2026-08-03 — antes só tinha `HasMaxLength(20)` sem `HasConversion`, o que fazia o EF gerar a coluna como `int`, não `nvarchar`; a migration antiga já dizia `nvarchar` só porque tinha sido escrita à mão/ficado desatualizada em relação ao `OnModelCreating` real — só percebemos ao regerar a migration do zero).
...
- **(2026-08-03)** Migrations antigas (`InitialCreate`, `AddRoleToUsers`, `AddPasswordToUsers`, `AddUpdatedAtToUsers`) **apagadas e substituídas por uma única baseline nova** (`InitialCreate`, gerada em 2026-08-03 com `dotnet ef migrations add --project src/Fiap.Workshop.Infrastructure --startup-project src/Fiap.Workshop.Api`), já cobrindo todos os agregados (`Users`, `Customers`, `Vehicles`, `InventoryItems`, `Services`, `ServiceOrders` + filhas). Decisão do usuário: como as migrations não são mais aplicadas em runtime (ver abaixo), não fazia sentido manter o histórico incremental de um schema que já mudou muito — a pasta `Migrations/` continua existindo só como apoio de ferramenta (`dotnet ef` gera SQL a partir do modelo real, útil para conferir/gerar o `db/init.sql`), não é mais a fonte de verdade em runtime.
- **(2026-08-03)** `Program.cs` **não chama mais** `dbContext.Database.MigrateAsync()`. O schema agora nasce pronto via `db/init.sql` (script SQL idempotente, único arquivo-fonte, com `CREATE DATABASE`/`CREATE TABLE IF NOT EXISTS` para todas as tabelas), executado pelo serviço `sqlserver-init` no `docker-compose.yml` **antes** da `api` subir (`depends_on: condition: service_completed_successfully`). Decisão explícita do usuário: projeto acadêmico, banco deve "nascer pronto" via Docker, sem depender de migration em runtime nem em testes. O mesmo `db/init.sql` é reaproveitado pelos testes de integração (`DatabaseFixture`, via `Link` no `.csproj`) — variável `$(DatabaseName)` (sintaxe `sqlcmd`) é substituída por `Fiap_Workshop` no compose (`-v DatabaseName=...`) e por `IntegrationTestsDb` via `string.Replace` no `DatabaseFixture` (que roda os batches por ADO.NET, não por `sqlcmd`, então não interpreta a variável nativamente).
- **(2026-08-03)** Modelos de infra criados para todos os agregados do domínio (seguindo o padrão de `UserModel`/`UserRepository`): `CustomerModel`, `VehicleModel`, `InventoryItemModel`, `ServiceModel`, `ServiceOrderModel` (+ `ServiceOrderPartModel`, `ServiceOrderServiceModel`, `ServiceOrderStatusHistoryModel` como filhos do mesmo agregado, com `HasMany().WithOne().OnDelete(Cascade)`), com `ModelMappers`/`DomainMappers` e configuração completa em `AppDbContext.OnModelCreating` (índices únicos em `Document/Email/Phone` do Customer, `LicensePlate`, `Code`, `Number`; FKs explícitas entre agregados — ex. `Vehicle.CustomerId → Customers` — como `HasOne<T>().WithMany().HasForeignKey().OnDelete(Restrict)`, sem navegação, pra não acoplar os agregados no código mas ainda ter integridade referencial no banco). `ServiceOrder` ganhou `AddParts/AddServices/AddStatusHistory` (recebem `IEnumerable<T>`) pra hidratar as coleções ao reconstruir o agregado a partir do banco. Repositories desses agregados ainda não foram implementados (só `IUserRepository`/`UserRepository` existem) — próximo passo natural.
- `Fiap.Workshop.Api.csproj` também referencia `Microsoft.EntityFrameworkCore.Design` (necessário porque a Api é o startup project usado pela ferramenta `dotnet ef` para gerar migrations, ainda útil como apoio de desenvolvimento mesmo não sendo mais aplicada em runtime).
...
### `Program.cs`
Pipeline: `AddControllers` (não usado, pois tudo é Minimal API — resquício de template), `AddApplication → AddInfrastructure → AddApi`, `MapOpenApi + UseSwaggerUI` (docs em `/swagger`, aponta pro JSON em `/openapi/v1.json`), `UseExceptionHandler`, `UseHttpsRedirection`, `UseAuthentication`, `UseAuthorization`, `UseSerilogRequestLogging`, `MapEndpoints()`.
