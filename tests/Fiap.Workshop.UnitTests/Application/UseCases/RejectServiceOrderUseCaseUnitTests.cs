...
public class RejectServiceOrderUseCaseUnitTests : LoggerTestBase<RejectServiceOrderUseCase>
{
    ...
    private readonly RejectServiceOrderUseCase _useCase;

    public RejectServiceOrderUseCaseUnitTests()
    {
        _useCase = new(
    ...
    private RejectServiceOrderInput CreateInput(Guid serviceOrderId) => new(
    ...
    public async Task Handle_ShouldSuccess_WhenServiceOrderIsAwaitingApprovalAndAllReservedPartsExist()
    {
        ...
        response.Status.Should().Be(ServiceOrderStatus.Cancelled.ToString());
