...
public class InventoryItemUnitTests
{
    ...
    [Fact(DisplayName = "InventoryItem >> Should release reservation >> When quantity does not exceed the reserved quantity")]
    public void InventoryItem_ShouldReleaseReservation_WhenQuantityDoesNotExceedTheReservedQuantity()
    {
        // Arrange
        var inventoryItem = CreateInventoryItem(20, 8, 5, 49.90m);
        var before = DateTime.Now;
    ...
    [Fact(DisplayName = "InventoryItem >> Should throw DomainException >> When releasing more than the reserved quantity")]
    public void InventoryItem_ShouldThrowDomainException_WhenReleasingMoreThanTheReservedQuantity()
    {
        // Arrange
        var inventoryItem = CreateInventoryItem(20, 8, 5, 49.90m);

        // Act
        var act = () => inventoryItem.ReleaseReservation(9);

        // Assert
        var exception = Assert.Throws<DomainException>(act);
        Assert.Equal(InventoryItemErrors.ReleaseQuantityExceedsReservedQuantity, exception.Message);
        Assert.Equal(20, inventoryItem.QuantityOnHand);
        Assert.Equal(8, inventoryItem.ReservedQuantity);
    }
}
