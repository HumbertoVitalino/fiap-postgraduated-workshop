...
public sealed class RejectServiceOrderUseCaseIntegrationTests(DatabaseFixture fixture)
{
    ...
    private async Task<Output> HandleAsync(RejectServiceOrderInput input)
    {
        using var scope = fixture.Services.CreateScope();
        var useCase = scope.ServiceProvider.GetRequiredService<IRejectServiceOrderUseCase>();
        return await useCase.Handle(input, CancellationToken.None);
    }

    [Fact(DisplayName = "RejectServiceOrderUseCase >> Should release stock and move to Cancelled >> When service order is AwaitingApproval")]
    public async Task Handle_ShouldReleaseStockAndMoveToCancelled_WhenServiceOrderIsAwaitingApproval()
    {
        // Arrange
        ...
        var input = new RejectServiceOrderInput(Guid.NewGuid(), serviceOrder.Id, attendantId);
        ...
        updated.Status.Should().Be(ServiceOrderStatus.Cancelled.ToString());
        ...
        foundOrder!.Status.Should().Be(ServiceOrderStatus.Cancelled);
        ...
        foundInventoryItem!.QuantityOnHand.Should().Be(20);
        foundInventoryItem.ReservedQuantity.Should().Be(0);
    }

    [Fact(DisplayName = "RejectServiceOrderUseCase >> Should fail >> When service order does not exist")]
    public async Task Handle_ShouldFail_WhenServiceOrderDoesNotExist()
    {
        // Arrange
        var attendantId = await CreateUserAsync(UserRole.Attendant);
        var input = new RejectServiceOrderInput(Guid.NewGuid(), Guid.NewGuid(), attendantId);
    ...
    [Fact(DisplayName = "RejectServiceOrderUseCase >> Should throw >> When service order is not AwaitingApproval")]
    public async Task Handle_ShouldThrow_WhenServiceOrderIsNotAwaitingApproval()
    {
        ...
        var attendantId = await CreateUserAsync(UserRole.Attendant);
        var input = new RejectServiceOrderInput(Guid.NewGuid(), created.Id, attendantId);
